            ...
            <div class="card-body mt-4">
                <table class="table table-bordered table-striped">
                    ...
                    <tbody>
                        ...
                        $query = "SELECT users.id_user, `username`, `email`, `created_at`,`avatar` FROM `users`INNER JOIN `employee` ON users.id_user = employee.id_user   WHERE `role`=0 LIMIT $begin,$limit  ";
    ...
    <nav  aria-label="Page navigation example mt-5">
        ...
        <ul class="pagination justify-content-center">
            <li class="page-item <?php if($page <= 1){ echo 'disabled'; } ?>">
                <a class="page-link" href="users-employee.php?page=<?= $page<=1 ? '' : $page - 1 ?>">&laquo;</a>
            </li>
            <?php
            for ($i = 1; $i <= $totoalPages; $i++) {
            ?>
                <li class="page-item <?= ($i == $page) ? 'active' : '' ?>">
                    <a class="page-link" href="users-employee.php?page=<?= $i ?>"><?= $i ?></a>
                </li>
            <?php
            }
            ?>
            <li class="page-item <?php if($page >= $totoalPages) { echo 'disabled'; } ?>">
                <a class="page-link" href="users-employee.php?page=<?= $page >= $totoalPages ? '#': $page + 1 ?>">&raquo;</a>
            </li>
        </ul>
    </nav>
</div>
